...
import { PROJECTS_BY_REGION as P, IC } from "@/lib/projects";
...
// 建案資料與 IC 圖示已移至 @/lib/projects（全站共用）
...
function Card({ p, idx, fav, onOpen }) {
  const [hov, setHov] = useState(false);
...
export default function V5() {
  ...
  const router = useRouter();
  const openProject = (proj) => router.push(`/projects/${proj.id}`);
  ...
  return (
    <div style={{ background:"#f8f4ec", minHeight:"100vh", fontFamily:"'Noto Sans TC',sans-serif" }}>
      ...
      <div style={{ maxWidth:840, margin:"0 auto", padding:"0 28px 100px" }}>
        ...
        {/* ═══ HERO ═══ */}
        {hero && <div style={{ marginBottom:48 }}><Hero p={hero} fav={fav} onOpen={()=>openProject(hero)}/></div>}
        ...
        {grid.length>0 && <>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:20, marginBottom:64 }}>
            {grid.map((p,i) => <Card key={p.id} p={p} idx={i} fav={fav} onOpen={()=>openProject(p)}/>)}
          </div>
        </>}
        ...
        {list.length>0 && <>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:4, marginBottom:40 }}>
            {list.map((p,i) => <Row key={p.id} p={p} i={i} fav={fav} onOpen={()=>openProject(p)}/>)}
          </div>
        </>}
